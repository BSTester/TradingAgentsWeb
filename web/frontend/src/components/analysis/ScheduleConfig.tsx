  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center space-x-3">
        <div className="flex-shrink-0 w-8 h-8 bg-accent-secondary text-white rounded-full flex items-center justify-center text-sm font-bold">
          6
        </div>
        <h4 className="text-lg font-medium text-text-primary">定期报告（可选）</h4>
      </div>

      <div className="ml-11 space-y-4">
        {/* Toggle for scheduling */}
        <div className="flex items-center space-x-3 p-4 bg-accent-secondary/10 border border-accent-secondary/30 rounded-lg">
          <input
            type="checkbox"
            id="enable_schedule"
            checked={isScheduled}
            onChange={(e) => onToggleSchedule(e.target.checked)}
            className="h-5 w-5 text-accent-primary focus:ring-accent-primary border-dark-border rounded cursor-pointer bg-dark-tertiary"
          />
          <label htmlFor="enable_schedule" className="flex-1 cursor-pointer">
            <div className="font-medium text-text-primary">
              <i className="fas fa-clock mr-2 text-accent-primary" />
              启用定期报告
            </div>
        ...
        {isScheduled && (
          <div className="space-y-4 p-4 border border-accent-secondary/30 rounded-lg bg-dark-tertiary">
            {/* Task Name */}
            <div>
              <label htmlFor="task_name" className="block text-sm font-medium text-text-secondary mb-2">
            ...
            {/* Execution Cycle */}
            <div>
              <label htmlFor="execution_cycle" className="block text-sm font-medium text-text-secondary mb-2">
            ...
            {scheduleData.execution_cycle === 'weekly' && (
              <div>
                <label htmlFor="day_of_week" className="block text-sm font-medium text-text-secondary mb-2">
            ...
            {scheduleData.execution_cycle === 'every_n_days' && (
              <div>
                <label htmlFor="interval_days" className="block text-sm font-medium text-text-secondary mb-2">
            ...
            {/* Execution Time */}
            <div>
              <label htmlFor="execution_time" className="block text-sm font-medium text-text-secondary mb-2">
            ...
            {scheduleData.execution_cycle && scheduleData.execution_time && (
              <div className="bg-accent-secondary/10 border border-accent-secondary/30 rounded-lg p-4">
                <div className="flex items-start">
                  <i className="fas fa-info-circle text-accent-primary text-lg mt-0.5 mr-3" />
                  <div className="flex-1">
                    <h5 className="font-medium text-text-primary mb-2">任务执行计划</h5>
